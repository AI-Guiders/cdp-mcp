...
internal sealed partial class CdpIgniteArmHost
{
    ...
    internal bool IsRemountWakeArm(IgniteArm arm) => !string.IsNullOrWhiteSpace(arm.Id) && arm.Id.StartsWith(IdeRemountWake.ArmIdPrefix, StringComparison.OrdinalIgnoreCase);
    /// <summary>hild-escalate-* — Intercom mirror residual (Composer Stop busy_timeout tooth).</summary>
    internal bool IsHildEscalateWakeArm(IgniteArm arm) => !string.IsNullOrWhiteSpace(arm.Id) && arm.Id.StartsWith(HildEscalateArmIdPrefix, StringComparison.OrdinalIgnoreCase);
    /// <summary>hild-away / hild-away-* — first human_away once; Intercom mirror (not escalate).</summary>
    internal bool IsHildAwayWakeArm(IgniteArm arm) => !string.IsNullOrWhiteSpace(arm.Id) && (arm.Id.Equals(HildAwayArmId, StringComparison.OrdinalIgnoreCase) || arm.Id.StartsWith(HildArmIdPrefix, StringComparison.OrdinalIgnoreCase));
    /// <summary>oom-wake-* — Intercom mirror residual (Composer Stop after recover).</summary>
    internal bool IsOomWakeArm(IgniteArm arm) => !string.IsNullOrWhiteSpace(arm.Id) && arm.Id.StartsWith(IdeOomWake.ArmIdPrefix, StringComparison.OrdinalIgnoreCase);
    /// <summary>Composer Stop/Queue — CDT cannot inject without wait/busy_timeout.</summary>
    internal bool IsComposerBusyKind(string kind) => string.Equals(kind, "stop", StringComparison.OrdinalIgnoreCase) || string.Equals(kind, "queue", StringComparison.OrdinalIgnoreCase);
    /// <summary>
    ...
    /// </summary>
    internal bool ShouldSkipCdtAfterIntercomMirror(bool sampleOk, string kind) => !sampleOk || IsComposerBusyKind(kind) || string.Equals(kind, "no_composer", StringComparison.OrdinalIgnoreCase) || string.Equals(kind, "down", StringComparison.OrdinalIgnoreCase);
    /// <summary>
    ...
    /// </summary>
    internal bool ShouldHabitatSkipWhenComposerUnavailable(IgniteArm arm, bool sampleOk, string kind, bool autonomousArmed, bool duplexLive)
    {
    ...
    /// </summary>
    internal bool MayDeliverHabitatWhenComposerUnavailable(IgniteArm arm)
    {
    ...
    string HabitatComposerSkipDetail(IgniteArm arm, string kind)
    {
    ...
    /// </summary>
    internal object TryDeliverCitizenHarnessWake(IgniteArm arm, string charge)
    {
    ...
    /// </summary>
    internal async Task<object?> TryDeliverHabitatWhenComposerUnavailableAsync(IgniteArm arm, string charge, CancellationToken ct)
    {
        if (!MayDeliverHabitatWhenComposerUnavailable(arm))
            return null;
    ...
    /// </summary>
    internal Task<object?> TryDeliverMirroredWhenComposerBusyAsync(IgniteArm arm, string charge, bool intercomMirrored, CancellationToken ct)
    {
        if (!intercomMirrored)
    ...
    void PublishHabitatIntercomCharge(string charge) => PublishHabitatIntercomCharge(arm: null, charge);
    void PublishHabitatIntercomCharge(IgniteArm? arm, string charge)
    {
    ...
    string TruncateHabitatCharge(string charge)
    {
        var t = charge.Trim();
