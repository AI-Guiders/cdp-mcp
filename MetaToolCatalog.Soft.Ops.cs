...
internal static partial class MetaToolCatalog
{
    ...
    Meta("cdp_ignite", "AutoIgnition continuity — Guest Autoi still CDT→Cursor Composer (requires --remote-debugging-port=9222); wake latch SSOT ignite-wake-LATEST.json (habitat|composer). Plain timer: PF duplex busy|composing → prefer habitat skip CDT; autonomous+idle PF → stamp habitat SSOT then Guest CDT fallthrough (0.5.532). System wakes (remount/HILD/OOM/tool) stay Composer adapter + Intercom mirror. Not Cognitive CDP. op=scene|probe|chats|send|arm|disarm|list|hygiene|plateau|continuity|resume|autonomous|hild|halt|await_partner. ARM: when=build_finished|test_finished|shell_finished|human_away|timer task= (TM label only). HILD: Composer text idle 30s on Voice → human_away once → AutoI wake (default ARMED; op=hild_off). Default charge=minimal: canonical wake text + amnesia/compaction postfix at fire (no TM body in composer). charge=custom only for legacy templates. last_once=/await_partner: fire once → awaiting latch when autonomous off; under autonomous last_once does not invent-ban — arm tips: insurance ≠ park while TM leaf started (0.5.537); continuity scene + canonical charge same (0.5.538). halt=stop-world (autonomous+HILD off, clear arms, await partner). autonomous default ARMED: auto LeafPlateau does not await_partner — seed-wake instead. Alias go=ignite_desk.", new
    {
        type = "object",
        properties = new
        {
