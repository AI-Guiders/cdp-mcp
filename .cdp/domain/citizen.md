...
- Host execute is sync seat place + buffer open/replace/**create** + plan REPL (`cmd=`) + **`build`** + **`test`** + **`mcp`** + **`kb`** + **`shell`** + **`debug`** + **`git`** + **`find`** + **`ide`** (wait lifecycle/outlet/`ShellHabitat.Run`/`DebugPlane`/git soft organ/`IdeFindChannel`/`IdeLanguageTools.DispatchBareAsync`, bounded) — not cockpit W-spray.
