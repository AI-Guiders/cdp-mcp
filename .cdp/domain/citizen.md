...
- Host execute is sync seat place + buffer open/replace + plan REPL (`cmd=`) + **`build`** + **`test`** (wait `IdeSessionLifecycle.BuildAsync`/`TestAsync`, bounded) — not cockpit W-spray.
- `@intent build` / `build path=` runs session build via host-execute. `@intent test` / `test path=` / optional `filter=` runs session tests. `go=build` / `go=test` only place the organ.
