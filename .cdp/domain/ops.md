...
- **2026-08-16 ADR-0032 L3c CDP lifecycle durable** — `cdp_deploy` background defaults `durable=true` → `DurableJobStore` + supervisor spawns `CdpMcp.exe --durable-job`; `cdp_build`/`cdp_test` opt-in `durable=true`. Poll `cdp_lifecycle_last`. Survives KillRunning/remount without `terminal_*` escape. 0.5.720+.
