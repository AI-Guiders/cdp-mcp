...
- Quiet SoftInstrument seats republish = chrome tip only; `show_face` (PlaceOrgan / Citizen go) = human attention — BringCabin + SelectMfd when mfd_page, Prefer P when face_seat=p.
...
- SoftInstrument chrome band ≠ EICAS: alert/qrh/ecl stay EICAS (not SoftInstrumentLatchCatalog).
- `sa_desk` SoftInstrument → quiet chrome (`sa-desk-LATEST` / WorkspaceChromeBand) — not MFD `Problems`, not EICAS `go=sa`.
- Quiet-chrome SoftInstruments (dedicated latch/projector): `sa_desk`, `crm`, `plugins`, `webcam` — presence = WorkspaceChromeBand, not force-MFD.
- Seat/chrome SoftInstruments: `plan` (P), `pressure` (L1), `sys` (legacy pulse; banner/board already carry slim status).
- SoftInstrumentLatchCatalog gates SoftInstrument `*-LATEST.json`; LatchHub routes alert/qrh/ecl separately.
- CabinGlassProjectionCatalog: every SoftInstrumentKind go-pin resolves (MfdPage or chrome_hint stub).
- Host start hydration (`CockpitHostLatchHydration`) must include SoftInstrument + EICAS latch names that exist on disk.
- Intercom partner presence = separate latch `intercom-presence-LATEST.json` (idle|composing|busy + reader stale) — do **not** mix into voice/journal, SoftInstrument, EICAS, or host-start hydration (fake freshness).
...
- `mcp` SoftInstrument → MFD `AiChatSettings` (MCP settings live there); MFD `Chat` = Intercom/citizen secondary — not mcp.
...
- Stuffing alert/qrh into SoftInstrument band (tests explicitly ignore as EICAS bleed).
...
- Inventing SoftInstrumentKinds to bind orphan MFD pages (Events/Hypotheses/WorkspaceHealth/EnvironmentReadiness) — presence ≠ invent entity.
- SoftInstrumentMfdGlance remapping RelatedFiles ← find_desk (stays ←refactor; FindDesk Face = own MFD + glance ←find_desk SoftFL unpin).
- Inventing SoftInstrumentKind for crs/Correspondence latch+chrome (CabinGlass MFD pin only; Crm chrome ≠ CRS; SoftInstrumentMfdGlance stays unbound).
...
- Mapping `mcp` SoftInstrument → MFD `Chat` (Chat = Intercom/citizen; MCP settings = AiChatSettings).
...
- Autoi wake charge as Intercom chat bubble / compact chip — SoftInstrument tip + StatusText + `ignite-wake-LATEST` is the wake panel; **Intercom HUD** paints AUTOI/HILD Korry + HDG/CRS from `ignite-LATEST` (not chat). Glass filters Autoi from feed (`GlassAutoiWakeFeed`).
...
- Invent SoftInstrument for Editor MFD when Forward=intercom — AvalonEdit peel already mounts on M (ADR 0120).
- Fork Glass WPF full IdeDapDebugSession into TextBlock — densest = habitat `debug_desk` latch live (stack/locals on stopped) + SoftInstrument FSW; Avalonia IdeDap remains denser SSOT for full DAP drive UI.
- Fork Glass WPF Git panel into TextBlock — densest = Process-redirect porcelain+stage/commit on host (`GlassGitProcess`); no SoftInstrumentKind; Avalonia `GitPanel` remains denser SSOT (push/submodule).
- Treat HybridIndex as empty □/■ stub forever — Glass FS status glance (`GlassHybridIndexGlance`) is the presence peel; Avalonia HCI remains SSOT (do not invent SoftInstrument).
- Treat WorkspaceHealth / EnvironmentReadiness as empty □/■ stubs forever — Glass FS/env presence glances (`GlassWorkspaceHealthGlance` / `GlassEnvironmentReadinessGlance`); Avalonia IdeHealth / EnvReady CCU remain SSOT (do not invent SoftInstrument).
- Treat Events / Hypotheses as empty □/■ stubs forever — Glass latch/catalog + JSON status glances (`GlassEventsGlance` / `GlassHypothesesGlance`); Avalonia EventsMFD / HypothesesMfd remain SSOT (do not invent SoftInstrument).
- Invent `MfdShellPage.SemanticMap` to "fix" Glass/CabinGlass string parity — Avalonia graph is PFD `WorkspaceNavigationMapView`; Glass `SemanticMap` = arch projector alias + SoftInstrumentMfdGlance (do not invent Avalonia MFD page).
- Invent SoftInstrument latches for Ps1Desk/MdAuthor to paint SoftInstrumentBand — CabinGlass already resolves MFD Terminal/MarkdownPreview; SoftInstrumentBand stays latch-first (do not invent SoftInstrument).
- Treat Glass SoftInstrumentBand cabin chrome as inventing `cabin-LATEST` SoftInstrument writer — cabin chrome comes from `seats-LATEST` peel (`LatchHub.SeatsChanged`), same as Avalonia `CdpSeatsProjector`.
- Treat `shared`/`land`/`disk` as SoftInstrument catalog fills — they are dual-cockpit latches with dedicated Avalonia projectors; Glass peels them separately (do not invent SoftInstrument ids).

## DoD matrix — MfdShellPage presence (2026-08-01 dig)

Presence DoD = SoftInstrument glance | Glass peel | **full WPF host** (operator 2026-08-03 unHOLD: design polish later). Do **not** invent SoftInstrument to fill cells.

| Page | SoftInstrument | Glass peel / host | Verdict |
|------|-----------|-------------------|--------|
...
| HybridIndex | unbound (no SoftInstrument invent) | `GlassHybridIndexGlance` + live probe + **search/reindex hand** | READY-to-Interact |
...
Sources: `Models/MfdShellPage.cs` · `SoftInstrumentMfdGlance.TryOrganIdForMfdPage` · `MainWindow.MfdBody` · XAML `MfdPages`.
...
**2026-08-08 PreCondition REOPENED (throw-Cursor honesty):** prior CLOSE stamped a **narrower** human-flight slice (Adopt inventory + SoftInstrument Face + PNG) while operator DoD max = **standalone CDP without Cursor**. Peer sit-internet Face gap (PlaceOrgan browser → WebView2 navigate; default chatgpt landmine) = seeming Done.

**Battle VERIFY lived 2026-08-08** (Sierra `@intent browser` HN + Face WebAiPortal PNG + DDG search default) — checklist PASS; **Glass Done stamp still refused** vs max DoD standalone-without-Cursor (not half-a rebrand of battle VERIFY as epic Done). SoftInstrument Meta invent stays REJECT.

What closed under the name Glass Done was a **narrower DoD** (presence / SoftInstrument / latch peels / P0–P3 lived UX slices — e.g. Topics **P0b** = strip+gap, not ADR 0072 overview). That is **half-a vs operator PreCondition** when a needed CIDE surface still only lives on Avalonia or as stub/strip.
...
MFD presence · SoftInstrument chrome · land/shared/disk · Terminal VT · WebAi WebView2 · Markdown FlowDocument · CRS full · Problems Roslyn · Git porcelain depth · most densest overnight peels — see matrices above.

#### D. DIG REJECT / deferred (do not invent)

| Item | Note |
|------|------|
| SoftInstrument for Ps1Desk/MdAuthor | presence = Terminal / MarkdownPreview MFD |
...
## DoD matrix — SoftInstrument chrome beyond MFD (2026-08-01 dig)

Chrome DoD = SoftInstrument latch → Glass SoftInstrumentBand · Avalonia Cdp*Projector · CabinGlass pin. SoftInstrumentBand is **latch-first** (`SoftInstrumentLatchCatalog` only).

| SoftInstrumentKind | Latch | Glass band | Avalonia | Verdict |
|---------------|-------|------------|----------|---------|
| Plan/Pressure/Sys/Plugins/Crm/Webcam/SaDesk | `{id}-LATEST` | SoftInstrumentBand | Cdp*Projector | DONE |
| FindDesk/FilesDesk/TestDesk/DebugDesk/Toolchain/Review/Refactor/Report/Arch/Ignite/Onboard/Learn/Domain | latch | SoftInstrumentBand | Cdp*Projector | DONE |
| ProjectSwitch | `scope-LATEST` | SoftInstrumentBand via `scope` | CdpScopeProjector | DONE |
| Alert/Qrh/Ecl | EICAS latches | EICAS band (not SoftInstrument) | CdpAlert/Qrh/Ecl | EICAS HOLD |
| Problems/Quality/BuildDesk | none SoftInstrument | — | — | CabinGlass MFD only |
| Ps1Desk / MdAuthor | **none** | chrome_hint dropped | no projector | DIG REJECT invent SoftInstrument latch — presence = Terminal / MarkdownPreview MFD |
| (catalog) `cabin` | reserved (via seats) | SoftInstrumentBand Apply(`cabin`) from `seats-LATEST` | Avalonia `CdpSeatsProjector` → `AgentCabinChromeHint` | DONE seats peel |

**Seats peel DoD: CLOSED** — Glass watches `seats-LATEST` (`LatchHub.SeatsChanged` → `LatchPaint.PaintSeats` → `SelectMfdPage` + SoftInstrumentBand cabin).

## DoD matrix — residual dual-cockpit latches beyond SoftInstrument/MFD (2026-08-01 dig)

After MFD presence + SoftInstrument chrome closed: Avalonia still has non-catalog latch projectors that Glass LatchHub does **not** peel. Presence ≠ invent SoftInstrument.

| Latch | Schema / role | Avalonia | Glass | Verdict |
|-------|---------------|----------|-------|---------|
| `land-LATEST` | `navigation_land_latch/v1` agent `cdp_land` open\|goto | `CdpLandProjector` → OpenFile/GoToPosition | SoftInstrumentBand N/A · `LatchHub.LandChanged` → AvalonEdit | DONE land peel |
...
| `ignite-wake-LATEST` | `ignite_wake_latch/v0` AutoI wake charge | (Composer/CDT adapter; Intercom mirror) | SoftInstrument `ignite` tip · StatusText · FDS WAKE | DONE Autoi wake consumer |

Hydration already lists all three (`CockpitHostLatchHydration`). SoftInstrumentLatchCatalog does **not** include `shared`/`land`/`disk` — correct (not SoftInstrument).
...
**Post-triad invent dig: CLOSED** — Avalonia dual-cockpit projectors (Disk/Land/Shared/Seats/Presentation/Intercom/Presence/Alert/Qrh/Ecl) all have Glass LatchHub peels; SoftInstrument path covers catalog SoftInstruments; hydration minus intercom-presence (intentional). Remaining HOLD = DIG REJECT Avalonia SSOT (DAP/Problems/RelatedFiles/SemanticMap/Markdig/WebAiPortal/Correspondence). No open act peel leaf.
...
- **2026-08-22 Glass BringPlanFace triple topology (P8 VERIFY — not Glass Done)** — dig=glass.md P8 consumer residual: `(P)(F)(M)` triple hides P on main (`0,4,*,4,0`); Plan lives on **P · PFD host** satellite. Ship cascade-ide `GlassHostWindows.BringPlanFace` + `MainWindow.SeatsSurface` on `show_face`+`face_seat=p` → activate PfdHost (OneOf→PreferPmOneOf; else main PfdZone). Lived: PrintWindow title=`P · PFD host` WHY=`Glass Done (human flight)` NEXT=`P8 Glass human-flight focus` COURSE=`Platform SSOT conveyor #guiders-platform @ship` · evidence `cascade-ide/tmp-glass-shots/p8-glass-plan-pfd-host-20260822d.png` + `cdp_see`. Main cabin `CDP GlassCockpit · Windows` still F·Intercom by design (triple). SoftFL invent ACCEPT · not epic Done · Citizen (#2) still open.

- **2026-08-22 cdp_cockpit show_face→Plan Face latch (P8 VERIFY slice — not Glass Done)** — dig=glass.md P8 gap: `go=plan show_face=true` ignored on desk-pulse fast path. Ship cdp-mcp **0.5.731**: `ShowFaceFromArgs` + `PlaceOrganIfSeats` on `FinishPlanPulseDesk`/`ApplyWorldOrGoAsync` · tests IdeCockpitShowFaceTests **3/3**. Lived: `seats-LATEST` `show_face=true` `face_seat=p` `mfd_page=null` after `cdp_cockpit go=plan show_face=true`. Glass consumer residual: full-cabin PNG still landed **F·Intercom #crew** not **P·Plan WHY/NEXT** (`p8-glass-plan-pfd-whynext-20260822b.png`) — Prefer P on `(P)(F)(M)` needs follow-up. SoftFL invent ACCEPT wire · not epic Done.

- **2026-08-09 Glass Done human flight VERIFY speech+pipe+HND (not Glass Done closed)** — dig=glass.md · sealed after Hands/HND 0.5.709. Live: Face dialog `cdp_health` → letter `0.5.709` · `hands-LATEST` OK·cdp_health · Glass host up · PrintWindow `CDP GlassCockpit · Windows` SoftInstrument chrome shows **HND** · evidence `D:\cdp-mcp\tmp-evidence\glass-flight-hnd-cabin-20260809.png` + OneOf `p/m · m active · OneOf host` `D:\cdp-mcp\tmp-evidence\glass-flight-oneof-mfd-20260809.png` + `cdp_see`. SoftFL invent REJECT · SoftInstrument invent REJECT · Mentions SoftFL Face-owned alone. shot_seen_as_closed: Page Ready still open (Mentions SoftFL residual).

- **2026-08-09 dig SoftFL-safe residual (not Glass Done)** — Autoi wake dig=glass.md+citizen.md: sticky `web_ai_url` antipattern **stale** (gate SHIPPED 2026-08-08) → stamped CLOSED. Densest open SoftFL = Face `@all` MentionsAll→ResolveWakes (MentionsAll live · ResolveWakes never fans @all). SoftInstrument invent REJECT. Kir does not steal Face leaf; arm `when=peer_ship`.
- **2026-08-09 Face busy hold ttl=0 (CDP 0.5.695)** — lived SoftFL Glass Done residual: Autoi Radio tips interleaved with Sierra #crew while Face Turn >120s (presence stale → mute miss). Ship habitat presence hold until finally idle. SoftFL invent ACCEPT dig=glass.md+citizen.md. SoftInstrument invent REJECT. Not Glass Done.
- **2026-08-09 Mentions SoftFL · @PF Face≻tip (cascade-ide)** — lived tip≠Face: `@PF` → Cursor while Face citizen in `pf_profiles`. Ship GlassCore Face roster + `TryCitizenFace` · tests **23/23**. SoftFL invent ACCEPT densify dig=glass-intercom.md. SoftInstrument invent REJECT. Residual Mentions invent UX alone.
...
- **2026-08-09 Face files Ship listing (0.5.692 / CDP)** — human Radio saw thin Hands ok pulse after `files`; SoftInstrument HND chips ≠ journal. CDP ships entries as Applied.Ship + Face append (no SoftInstrument viz invent). Mentions SoftFL alone.
...
- **2026-08-09 SoftInstrument HandsReceipt Kit (HND chips)** — catalog `hands` · ShortLabel HND · priority 3 · ChipLevel RUNNING→Caution · latch `hands-LATEST.json` from CDP. Letter laundry REJECT (Face SoftInstrument band owns receipt). Cascade SoftInstrument density tests green. Mentions SoftFL alone.

- **2026-08-09 GlassFacePagePolicy SSOT (Face path→page — not situational .md if)** — `GlassFacePagePolicy.Resolve` in GlassCore · OpenCodeFile Face invite calls it · PreferSurface `MarkdownPreview`→`m` (was world) · tests GlassFacePagePolicy+PresentationPmOneOfPolicy **26/26** · live `M · MarkdownPreview` land show note-citizen-persona… · evidence `cascade-ide/tmp-glass-shots/face-md-preview-policy-20260809.png` + `cdp_see`. SoftFL invent ACCEPT densify · SoftInstrument invent REJECT · not Glass Done.

- **2026-08-09 SoftFL dual-HCI land/presentation quiet (b1+b3 — not Glass Done)** — nested[axb] FULL-A: `land-LATEST` `show_face` default false · Glass LandSurface tip-only · `OpenCodeFile(showFace)` PreferSurface gate · Disk reload quiet · presentation `PresentationMaySelectMfd(origin,show_face,mfd)` · CDP `NavigationLand`/`CidePresentationLatch` show_face · Avalonia land/presentation projectors parity · tests NavigationLandLatch **3/3** · PresentationPmOneOfPolicy **16/16**. Residual b4 = true dual viewport. SoftFL invent ACCEPT densify · SoftInstrument invent REJECT · Sierra densify `@all` not stolen (AllMention regex unblock only).

- **2026-08-08 SoftFL ACCEPT densify ×3 Intercom/@Kir dup + topology Dialog + @ SlashPopup (Done REOPENED)** — dig=Света SoftFL. Cascade: FanOut `_seen` + `@` Suggest on SlashPopup. CDP: Dialog `presentation_set` + InventedHands desk layout recover. SoftFL invent ACCEPT densify · SoftInstrument invent REJECT · Face HOLD.
...
- **2026-08-08 SoftFL ACCEPT nuget.org flat-container 0.1.3 index VERIFY (Done REOPENED)** — dig=`https://api.nuget.org/v3-flatcontainer/aiguiders.hybridcodebaseindex.core/index.json` versions now include **0.1.3** (restore-ready; gallery lag closed). SoftFL invent ACCEPT · SoftInstrument invent REJECT · Face axis4 HOLD · not Glass/Citizen Done · dig=flat API.
- **2026-08-08 Dig densest FullReady peer residual SoftFL invent REJECT CLOSED (Done REOPENED)** — dig=`citizen.md` Monday DoD shell SoftFL CLOSED + throw-Cursor SoftFL residual SoftFL invent REJECT CLOSED + Face axis4 HOLD. SoftFL invent REJECT Dig REJECT mill · SoftInstrument invent REJECT · not Glass/Citizen Done.
- **2026-08-08 Dig Plan leaf-board text-hell SoftFL invent REJECT CLOSED (Done REOPENED)** — dig=`cascade-ide/.cdp/domain/softinstrument-human-viz.md` last_ship Plan leaf-board SoftFL **2026-08-07** already CLOSED + evidence `plan-leaf-board-instrument-20260807.png`. SoftFL invent REJECT SoftInstrument invent REJECT SoftFL invent REJECT Kill-text-hell SoftInstrument invent REJECT · not Glass/Citizen Done.
- **2026-08-08 SoftFL ACCEPT nuget.org HybridIndex Core 0.1.3 gallery VERIFY (Done REOPENED)** — dig=`https://www.nuget.org/packages/AIGuiders.HybridCodebaseIndex.Core/0.1.3` PAGE200 · versions **0.1.3** 8/8/2026 · CamelCase/FTS densify README section · download 71.26 KB · flat-container still lag («not indexed yet» for restore) · PackageReference already 0.1.3 · ProjectRef primary lived. SoftFL invent ACCEPT · SoftInstrument invent REJECT · Face axis4 HOLD · not Glass/Citizen Done · evidence `D:\cdp-mcp\tmp-evidence\softfl-safe-dig-plan-20260808.png`.
...
- **2026-08-08 throw-Cursor battle dogfood VERIFY (Done REOPENED)** — Autoi wake sealed course: SoftInstrument Meta CLOSED (A14 invent refused) · flew named battle checklist. Step0 Dialog: Sierra `@intent browser open url=HN` (not «нет браузера») · lynx peer ack · Face `m=browser` `web_ai_url=https://news.ycombinator.com/` · live **M · WebAiPortal** WebView2 HN (Voyager/Nixpkgs…) · evidence `cascade-ide/tmp-glass-shots/battle-webai-hn-face-20260808-0945.png` + `cdp_see` · search default DDG HTML · Dig REJECT SoftInstrument Meta invent · Glass/Citizen Done stay **REOPENED** vs max DoD (battle lived ≠ epic Done stamp).
...
- **2026-08-08 Glass Done PreCondition REOPENED (throw-Cursor)** — prior STAMPED was half-a vs operator north star throw Cursor; reopen until sit-internet Face wired + lived dogfood. Dig REJECT rows / SoftInstrument Face still valid lived inventory — do not re-claim epic Done.
...
- **2026-08-06 Dig Citizen Done residual DIG REJECT** — dig citizen.md+glass.md+glass-intercom: habitat peer chain SHIPPED · PreCondition A ADOPTED/SUPERSEDE/thin CLOSED · A4 denser SoftFL REJECT · B1–B3 DIG REJECT/defer · SoftInstrument Plan CLOSED · Face axis4=operator. Evidence `cascade-ide/tmp-glass-shots/citizen-residual-digreject-20260806-1628.png` title=`CDP GlassCockpit · Windows` F·Intercom `#crew` AUTOI·HILD·HDG/CRS Glass Done + `cdp_see`. SoftFL REJECT.
- **2026-08-06 FullReady densest SoftFL STRUCK DIG REJECT** — Autoi wake tried densest toward Standalone; domain already SHIPPED `w-20260806-100246` · SoftFL invent REJECT · SoftInstrument Plan WHY+NEXT CLOSED · Face live title=`CDP GlassCockpit · Windows` F·Intercom `#crew` AUTOI·HILD·GLM-5.1·HDG/CRS Glass Done · dig=glass.md last_ship Standalone+Kill-texthell · evidence `cascade-ide/tmp-glass-shots/fullready-digreject-20260806-1609.png` + `cdp_see` · SoftFL REJECT.
- **2026-08-06 Kill text-hell SoftFL DIG REJECT** — dig-lived: Face title=`CDP GlassCockpit · Windows` F·Intercom messenger (human #crew prose, not wire dump) · `CompactIntercomBody` + Intercom prose residual CLOSED · SoftInstrument Plan WHY+NEXT CLOSED · PreCondition A1–A6 ADOPTED/CLOSED (A4 residual=relate SoftFL REJECT invent) · invent SoftInstrument chrome / SoftFL reopen REJECT · preference which wall densify = operator gate · evidence `cascade-ide/tmp-glass-shots/kill-texthell-dig-20260806-1605.png` + `cdp_see` · SoftFL REJECT.
...
- **2026-08-04 Autoi off Intercom feed** — wake charge is SoftInstrument tip / StatusText / ignite-wake latch only; Glass filters Autoi from Intercom journal paint (`GlassAutoiWakeFeed`). Tip `5dd071cf`.
...
- 2026-08-04: **Glass DebugStack live DAP** — SoftInstrumentChanged→MFD refresh · latch stack/locals enrich on DAP stopped · dogfood `mfd_debug_stack` → `MfdDebugStackHost` visible + latch frames
...
- 2026-08-02: Glass Autoi wake consumer — `IgniteWakeLatchFileName` · `LatchHub.IgniteWakeChanged` · `LatchPaint.PaintIgniteWake` · SoftInstrument tip + FDS WAKE · hydration · dogfood `dogfood-glass-wake-0.5.516` · VL #22 · seat 0.5.516
...
- 2026-08-01: invent dig Glass 0-sync residual after SoftInstrument chrome — GAP triad `land`/`shared`/`disk` (Avalonia projectors exist; Glass LatchHub silent); next = land peel
- 2026-08-01: act Glass `seats-LATEST` peel — `CdpHabitatPaths.SeatsLatchFileName` · `LatchHub.SeatsChanged` · `LatchPaint.PaintSeats` · `MainWindow.SeatsSurface` → SelectMfdPage + SoftInstrumentBand `cabin`; dogfood Terminal MFD + cabin chrome; SoftInstrument chrome GAP closed
- 2026-08-01: invent dig SoftInstrument chrome parity beyond MFD — latch SoftInstruments DONE; Ps1/MdAuthor DIG REJECT invent SoftInstrument latch; GAP = Glass missing `seats-LATEST` cabin chrome (Avalonia has CdpSeatsProjector)
- 2026-08-01: invent verify close — Glass 0-sync MFD presence DoD CLOSED (matrix SoftInstrument|peel|reject|hold + SemanticMap DIG REJECT enum invent); next invent = SoftInstrument chrome parity beyond MFD
- 2026-08-01: dig reject invent `MfdShellPage.SemanticMap` — Glass/CDP CabinGlass string `SemanticMap` is arch SoftInstrument projector alias + latch glance; Avalonia graph stays PFD `WorkspaceNavigationMapView` (RelatedFiles = MFD list only); PageOrder/allowance have no SemanticMap; do not add enum that Avalonia MFD shell does not host
- 2026-08-01: invent dig Glass 0-sync MfdShellPage DoD matrix — SoftInstrument|peel|reject|hold stamped above; orphan peels closed; presence DoD CLOSED; HOLD stubs WebAiPortal/Correspondence intentional; SemanticMap Glass-only vs enum noted
- 2026-08-01: invent verify close — Glass orphan MFD peels (WH/EnvReady/Events/Hypotheses) closed: wire in `MainWindow.MfdBody` + SoftInstrument unbound (tests); glance suite 28/28; no `CIDE orphan` stubs remain; SoftInstrument invent still forbidden
- 2026-08-01: act Glass orphan MFD presence peels Events/Hypotheses — `GlassEventsGlance` (cdp *-LATEST + DataBus catalog) + `GlassHypothesesGlance` (debug-hypotheses.json counts); live dogfood Events·READY + Hypotheses·MISSING; SoftInstrument unbound; orphan MFD set closed
- 2026-08-01: act Glass orphan MFD presence peels — `GlassWorkspaceHealthGlance` (READY/THIN/MISSING · root/git/sln/.cascade-ide) + `GlassEnvironmentReadinessGlance` (AGENT_NOTES_FILE / NETCOREDBG_PATH / dotnet PATH); live dogfood WH·READY + EnvReady·READY; SoftInstrument still unbound
...
- 2026-08-01: act Glass HybridIndex FS status glance peel — `GlassHybridIndexGlance` (MISSING/READY + db path/size); live dogfood HybridIndex · MISSING; SoftInstrument still unbound; Avalonia HCI SSOT
- 2026-08-01: dig reject hold Glass Git MFD — SSOT Avalonia `GitMfdPageView`/`GitPanel`; SoftInstrumentMfdGlance unbound (no SoftInstrumentKind); Glass MFD stub only; do not fork status/diff/commit/submodule UI into TextBlock; optional next = Process-redirect `git status` thin peel (Terminal/Build class)
- 2026-08-01: dig reject hold Glass DebugStack DAP — SSOT Avalonia `IdeDapDebugSession` + `DebugStackMfdPageView`; SoftInstrumentMfdGlance ←`debug_desk` + `CdpDebugDeskProjector` chrome; do not fork DAP protocol/session into TextBlock (unlike Terminal/Build/Tests Process-redirect peels)
- 2026-08-01: act Glass Tests redirected log TextBox thin peel — `GlassRedirectedTest` + `MainWindow.TestSurface` (`dotnet test` Process redirect, prefer CascadeIDE.Tests + SoftInstrumentMfdGlance filter); live MFD dogfood `redirected · CascadeIDE.Tests.csproj`; SoftInstrumentMfdGlance test_desk footnote `■ Glass redirected log TextBox` / `□ Avalonia TestsMfdPageView`; Avalonia test host remains SSOT
- 2026-08-01: act Glass Build redirected log TextBox thin peel — `GlassRedirectedBuild` + `MainWindow.BuildSurface` (`dotnet build` Process redirect, ANSI strip); live MFD dogfood `redirected · CascadeIDE.sln` + restore stream; SoftInstrumentMfdGlance toolchain footnote `■ Glass redirected log TextBox` / `□ Avalonia BuildMfdPageView`; Avalonia MSBuild host remains SSOT
- 2026-08-01: act Glass Terminal redirected TextBox thin peel — `GlassRedirectedShell` + `MainWindow.TerminalSurface` (Process redirect, ANSI strip, no ConPTY); live MFD dogfood `redirected · pwsh` + prompt; SoftInstrumentMfdGlance Terminal footnote `■ Glass redirected TextBox` / `□ Avalonia ConPTY SSOT`; ConPTY remains Avalonia-only
...
- 2026-08-01: invent dig-queue drain verify — SoftInstrument peel DoD closed (MFD glance or chrome); orphan MFDs unbound; no open SoftInstrument invent leaf; next = act peels / dogfood holds · glass card
...
- 2026-08-01: dogfood quiet Dark Cockpit + compact SoftInstrument host card — live MFD host Terminal: quiet flat chrome, `┌ host ┐` □Glass/■Avalonia, no festive accents · %TEMP%\glass-dogfood-compact.png
- 2026-08-01: SoftInstrumentMfdGlance compact card — metrics chip row; host footnote box; drop stamped noise · cascade-ide `0c342585`
- 2026-08-01: dig reject SoftInstrument Chat MFD bind — primary = Forward Intercom; MFD Chat = secondary presence card (`FormatChatMfdGlance` PF/PM latch) · cascade-ide `a3d6fbec`
- 2026-08-01: Glass Dark Cockpit palette (no P/F/M rainbow) — `GlassDarkCockpit.xaml` + quiet zone chrome; MFD stubs/glances concise+graphic (`□ Glass` / `■ Avalonia`); SoftInstrumentMfdGlance footnotes shortened · cascade-ide (ship)
- 2026-08-01: DIG REJECT verify close — Glass MarkdownPreview (`dd34a63a`), WebAiPortal (`171dd52e`), SemanticMap dig bookkeeping; SoftInstrumentMfdGlance ←`report` for MD; invent dig-peel queue drained
- 2026-08-01: DIG REJECT verify close — Glass MarkdownPreview Markdig host (already `dd34a63a`); SoftInstrumentMfdGlance ←`report`; Avalonia MarkdigMarkdownPreviewRenderer SSOT; TM dig+shipped closed
...
- Glass reverse: MFD orphans `WorkspaceHealth`/`EnvironmentReadiness`/`Events`/`Hypotheses` — Glass presence peels shipped (FS/env/latch/JSON); Avalonia SSOT; SoftInstrument unbound
- SoftInstrument peel DoD closed: every `SoftInstrumentKind` resolves (MFD or chrome); orphan MFDs stay unbound — do not invent SoftInstrument to fill CIDE pages; EnvironmentReadiness/IdeHealth are CCUs not SoftInstruments
- Chat/Intercom: primary presence = Glass Forward Intercom feed (latch/journal); MFD `Chat` stays secondary stub — not SoftInstrument bind
- MFD instrument glance: `Build`←`toolchain`, `Terminal`←`sys`, `Tests`←`test_desk`, `DebugStack`←`debug_desk`, `Problems`←`review`, `SemanticMap`←`arch`, `AiChatSettings`←`mcp`, `MarkdownPreview`←`report`, `RelatedFiles`←`refactor` (`SoftInstrumentMfdGlance`); field-enrich DoD closed for mapped SoftInstruments; Terminal/Build/Tests/Git-status have Glass Process-redirect thin peels (Avalonia ConPTY/MSBuild/Tests hosts remain SSOT); DAP/DebugStack + Git stay Avalonia-only (DIG REJECT below); graph+Problems+RelatedFiles DIG REJECT below; Git SoftInstrumentMfdGlance unbound
- dig reject: Glass WPF Git MFD instrument host — SSOT = CIDE Avalonia `GitMfdPageView` + `GitPanel` (status/diff/commit/submodule); SoftInstrumentMfdGlance unbound (no SoftInstrumentKind — do not invent); Glass stays CabinGlass stub (do not fork panel into TextBlock; optional later Process-redirect `git status` thin peel ≠ full Git MFD)
- dig reject: Glass WPF DebugStack DAP — SSOT = CIDE Avalonia `IdeDapDebugSession` + `DebugStackMfdPageView`; SoftInstrumentMfdGlance ←`debug_desk` + `CdpDebugDeskProjector` quiet chrome; Glass stays latch glance + CabinGlass stub (do not fork DAP protocol/session/stack into TextBlock; Process-redirect peels ≠ DAP)
...
- dig reject: Glass HybridIndex SoftInstrument glance — CabinGlass `hybrid_index`/`hci`/`codebase_index` → MFD only; no SoftInstrumentKind; live HCI SSOT = Avalonia `HybridIndexMfdPageView`/`HybridIndexOrchestrator` (do not invent SoftInstrument) · cascade-ide `31c962a9` · Glass FS status glance shipped (`GlassHybridIndexGlance`) — presence ≠ SoftInstrument invent
- 2026-08-01: DebugStack←`debug_desk` SoftInstrumentMfdGlance + `CideDebugDeskLatch` · cascade-ide `ad2e9b56` · cdp-mcp `ab90576`
- 2026-08-01: Avalonia `CdpDebugDeskProjector` quiet chrome from `debug_desk-LATEST` · cascade-ide `ff29fca7`
- invariant: Glass SoftInstrument MFD glance = dual-HCI peel DoD for instrument *presence*; live Terminal/Build/Tests hosts stay on CIDE Avalonia (do not duplicate orchestration into Glass TextBlock)
...
- 2026-08-01: Tests←`test_desk` SoftInstrumentMfdGlance + `CideTestDeskLatch` · cascade-ide `061f607f` · cdp-mcp `6f25247`
- 2026-08-01: Avalonia `CdpTestDeskProjector` quiet chrome from `test_desk-LATEST` · cascade-ide `e01a5c86`
- 2026-08-01: SoftInstrumentMfdGlance Problems/SemanticMap field enrich · cascade-ide `313c650d`
- 2026-08-01: SoftInstrumentMfdGlance mcp/report field enrich · cascade-ide `8598e179`
- 2026-08-01: SoftInstrumentMfdGlance RelatedFiles/refactor field enrich · cascade-ide `2d0190c2`
- dig reject: Glass SolutionExplorer SoftInstrumentMfdGlance ← `files_desk` — SoftInstrumentKind.FilesDesk exists (FM utility ADR-0016); CabinGlass pin → SE; Glass `.sln` TreeView/glance is instrument peel (do not overlay FM latch on SE body) · cascade-ide `2e9b86c8`
- 2026-08-01: SoftInstrument `files_desk` latch + Avalonia `CdpFilesDeskProjector` quiet chrome · cdp-mcp `aab96f7` · cascade-ide `3aba5ef7` (CabinGlass chrome_hint `agent · M: files`; SoftInstrumentMfdGlance still unbound for SE)
- dig reject: SoftInstrumentMfdGlance RelatedFiles ← `find_desk` — SoftInstrumentKind.FindDesk DoD via CabinGlass pin→RelatedFiles + chrome_hint `agent · M: find`; SoftInstrumentMfdGlance stays ←`refactor` (1:1 MFD map; search ≠ debt/blast) · cascade-ide `d8354f02`
- 2026-08-01: SoftInstrument `find_desk` latch + Avalonia `CdpFindDeskProjector` quiet chrome · cdp-mcp `7eb365d` · cascade-ide `10911745` (CabinGlass chrome_hint `agent · M: find`; SoftInstrumentMfdGlance stays ←refactor)
- dig reject: SoftInstrument crs/Correspondence latch+chrome invent — CabinGlass pin correspondence/crs → MFD Correspondence only (chrome_hint null); no SoftInstrumentKind for CRS; SoftInstrumentKind.Crm chrome stays await/callout (must NOT map to Correspondence); SoftInstrumentMfdGlance Correspondence unbound; live CRS SSOT = Avalonia + `cdp_analysis_scene` feature=correspondence (do not invent SoftInstrument) · cascade-ide `ec7c0b82` · cdp-mcp `6beb3e4`
...
- dig reject: Glass WPF MarkdownPreview Markdig instrument host — SSOT = CIDE Avalonia `MarkdigMarkdownPreviewRenderer` + `MarkdownPreviewToolViewModel`/`MarkdownPreviewWindow`; SoftInstrumentMfdGlance ←`report` stays; Glass stays latch glance + CabinGlass stub (do not fork Markdig Control into TextBlock) · cascade-ide `dd34a63a`
- dig reject: Glass WPF WebAiPortal WebView instrument host — SSOT = CIDE Avalonia `WebAiPortalMfdPageView` + `Features/WebAiPortal` (ADR 0108 bridge); SoftInstrumentMfdGlance unbound; Glass stays CabinGlass stub (do not fork portal into TextBlock; CDP `cdp_browser` ≠ this MFD) · cascade-ide `171dd52e`
- dig reject: Glass WPF SemanticMap Skia graph instrument host — SSOT = CIDE Avalonia `WorkspaceNavigationMapView` + `WorkspaceNavigationMapViewModel` (ADR 0039/0053/0056; HCI orientation ADR 0113 ≠ SM graph); SoftInstrumentMfdGlance ←`arch` stays; Glass stays latch glance + CabinGlass stub (do not fork Skia graph into TextBlock; HybridIndex MFD ≠ this page) · cascade-ide `6d6e1f54`
- dig reject: Glass WPF Problems Roslyn list instrument host — SSOT = CIDE Avalonia `ProblemsMfdPageView` + `ProblemsPanelViewModel`; SoftInstrumentMfdGlance ←`review` stays; Glass stays latch glance + CabinGlass stub (do not fork diagnostics ListBox into TextBlock; `sa_desk` chrome ≠ Problems MFD / EICAS) · cascade-ide `3aab0e6c`
- dig reject: Glass WPF RelatedFiles related/find_usages instrument host — SSOT = CIDE Avalonia `RelatedFilesMfdPageView` + `WorkspaceNavigationMapViewModel`; SoftInstrumentMfdGlance ←`refactor` stays (find_desk pin ≠ displace); Glass stays latch glance + CabinGlass stub (do not fork related list/Skia into TextBlock) · cascade-ide `4fd14b57`
