...
- **L6 Cursor SDK local (AxB · ADR-0232):** `harness=cursor` fires via `@cursor/sdk` `Agent.create|resume` + `send` when configured; CDT Composer (:9222) is **escape** (`delivery_path=cdt_escape`). `force_cdt` / `fire_policy=composer|cdt` skips SDK. Wake latch channel `sdk_local` + optional `sdk_agent_id`. No Connection Problems watch on `sdk_local` submit.
- **Citizen Autoi consume (0.5.551→0.5.554):** only when Composer unavailable (`TryDeliverHabitatWhenComposerUnavailableAsync` · gone/down) + `invite_ready` → `TryDeliverAutoiWake` · `detail=prefer_citizen` · Intercom `kind=citizen`. Idle PF + invite while Composer present → Guest CDT→Composer. **0.5.690:** prefer_citizen also `TryApplyCitizenFocusLane` → Face Who lane (tip≠Face).
...
- Remount wakes (`remount-wake-*`): Intercom mirror when PF idle; **mute Radio when PF Face busy|composing** (`IsHabitatPartnerLive` · 0.5.693 — lived remount thrash mid-Sierra Turn). **prefer_citizen** same mute (0.5.694): Composer unavailable + invite ready → defer Completions/Radio when Face busy (`prefer_citizen_face_busy`). CDT→Composer fallthrough intact. HILD escalate/away, OOM, tool-wake: still Intercom mirror even when PF busy|composing (`detail=escalate_intercom`|`hild_intercom`|`oom_intercom`|`tool_intercom`); prefer habitat stays off. Event wakes (build/test/shell): no mirror.
...
- **2026-08-22 ignite chat latch fire-time (ADR-0200)** — AutoI no longer injects into foreground Composer when `chat` null: arm stamps `conversation_id` + resolves `chat` from per-conversation `CdpTenantComposerLatch` at arm+fire; CDT `ConnectPageAsync` focuses target chat before `ComposerScoped` probe (`chat_not_found` if miss). `cdp_context composer="<title>"` per chat. Tests `IdeIgniteChatResolveTests`.
- **2026-08-22 TimerLoop leaf-fly per-tenant (ADR-0200)** — autonomous `ProbeFlight`+`PullForwardLongWorkTimersOnLeafFly` no longer global legacy TM bleed: distinct `TenantWire` on armed last_once timers → enter slice → probe → pull matching arms only (`TryPullForwardLongWorkTimersOnLeafFlyPerTenant` · `ArmTenantWireEquals`). Tests `IdeIgniteTenantPullForwardTests`.
...
- **2026-08-22 wake tier hygiene** — `charge=minimal` on arm caps `ComposeWakeBody` at Minimal even when TM preflight is Full (lifecycle `test_finished`/`build_finished` no longer dump Human-face). Full tier skips `ChargeHumanFacePostfix` when sealed course has Glass+Citizen DEFERRED (axes stay in `course=` sidecar). `IsMinimalChargeMode` · `IsGlassCitizenDeferredInSealedCourse` · `ChargeTemplateRev=20260822-wake-minimal-arm-cap-deferred-glass`.
- **2026-08-22 ignite arm tenant fire (ADR-0200)** — `TenantWire` stamped at arm from `CurrentSlice`; `RunFireAsync` enters tenant profile+execution scope via `BindTenantResolver`; background/lifecycle wakes read correct TM.
- **2026-08-22 joint course v0.5.735** — sealed course = team+agent joint; agent disputes with arguments (Scrum PO); operator considers + last word; silent rewrite ≠ dispute. being.md + playbook. — `ChargeOwnershipPostfix` + sealed course `HasOwnershipAxis`; diagnosis-only = seeming; operator ≠ patch queue. being.md + playbook § Твоё — твоё.
...
- **2026-09-19 L6 Cursor SDK local Autoi primary (ADR-0232)** — SDK `Agent.send|resume` is the Autoi gun; CDT Composer is escape with `delivery_path` ack; wake latch `channel=sdk_local`.
