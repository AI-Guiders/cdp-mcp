...
- After Intercom mirror (remount / escalate / hild-away / OOM / tool-wake / idle-PF) + Composer Stop/Queue **or** gone (`no_composer`/`down` / sample fail): habitat deliver + skip CDT (`detail=*_composer_busy`|`*_composer_gone`) — no busy_timeout→requeue / no_agent_composer thrash. Voice/send Composer: CDT fallthrough.
