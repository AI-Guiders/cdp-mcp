...
- Remount wakes (`remount-wake-*`): Intercom mirror when PF idle; **mute Radio when PF Face busy|composing** (`IsHabitatPartnerLive` · 0.5.693 — lived remount thrash mid-Sierra Turn). CDT→Composer fallthrough intact. HILD escalate/away, OOM, tool-wake: still Intercom mirror even when PF busy|composing (`detail=escalate_intercom`|`hild_intercom`|`oom_intercom`|`tool_intercom`); prefer habitat stays off. Event wakes (build/test/shell): no mirror.
