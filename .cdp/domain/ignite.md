...
- **2026-08-09 peer_ship Autoi wake (0.5.697 · HIH SoftFL)** — lived: timer ≠ «подожди пока Face шипнет». Ship: `when=peer_ship` (+leaf_done|ship aliases) · `NotifyPeerShip` from TM done/shipped · Intercom explicit ship cue (citizen|guest; CoT dump alone ≠ fire). SoftFL invent ACCEPT dig=IdeIgniteArmHost.PeerShip. AW §5a stamped. SoftOrgan invent REJECT.
