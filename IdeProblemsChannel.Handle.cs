...
{
    public static object Handle(DocumentBufferStore store, SessionContext session, IReadOnlyDictionary<string, JsonElement>? args = null)
    {
        ...
        var aimWanted = BoolOr(merged, "aim", defaultValue: true) && (wire is { Length: > 0 } || rowKey is { Length: > 0 });
        if (aimWanted)
        {
            if (wire is { Length: > 0 })
            {
                aimWire = wire;
                picked = snap.Rows.FirstOrDefault(r => string.Equals(r.Anchor, wire, StringComparison.OrdinalIgnoreCase));
            }
        ...
        var lines = snap.Rows.Take(24).Select(r => $"{Glyph(r.Severity)}{r.Id} {Path.GetFileName(r.Path)}:{r.Line} {r.Message}").ToArray();
        if (lines.Length == 0)
            lines = [snap.DiagnosedCount == 0 ? "(no cached diags — cdp_buffer op=diagnostics on open files)" : "(clear)"];
        return new
        {
            ...
            rows = snap.Rows.Select(r => new { id = r.Id, severity = r.Severity, message = r.Message, code = r.Code, path = Rel(root: session.ProjectRoot, r.Path), line = r.Line, end_line = r.EndLine, anchor = r.Anchor, doc_id = r.DocId, stale = r.Stale, go = "problems", go_args = new { row = r.Id, aim = true } }).ToArray(),
            picked = picked is null ? null : new
            {
                id = picked.Id,
            ...
            hint = snap.Rows.Count == 0 ? "Empty list. Warm: open buffers → diagnostics; then go=problems. Gates stay go=quality." : "Tap: go=problems go_args.row=p1 (or wire=). Aims sniper corridor+peek — not path:line chat."
        };
    }
    ...
    static object[] BuildNext(Snap snap, Row? picked)
    {
        ...
        {
            list.Add(new { go = "problems", label = $"Aim {snap.Rows[0].Id}", why = "go_args.row=" + snap.Rows[0].Id });
        }
    ...
    static bool TryPickRow(IReadOnlyList<Row> rows, string key, out Row row)
    {
        ...
        var hit = rows.FirstOrDefault(r => string.Equals(r.Id, k, StringComparison.OrdinalIgnoreCase));
        if (hit is null)
            return false;
        row = hit;
        return true;
