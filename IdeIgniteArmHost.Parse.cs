...
internal sealed partial class CdpIgniteArmHost
{
    ...
    bool TryCreateArm(IReadOnlyDictionary<string, JsonElement> args, out IgniteArm arm, out object? err)
    {
        ...
        arm = new IgniteArm
        {
            ...
            SdkAgentId = Opt(args, "sdk_agent_id") ?? Opt(args, "agent_id"),
            ForceCdt = OptBool(args, "force_cdt") ?? false,
            CursorFirePolicy = Opt(args, "fire_policy") ?? Opt(args, "cursor_fire_policy"),
            Port = port,
            Once = once,
            LastOnce = lastOnce,
            OkOnly = okOnly,
