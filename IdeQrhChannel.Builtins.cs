    ...
    [
        ...
        new(
            ...
            [
                ...
                new("Not connected + exe still up: terminal_* Recover-CdpSeatRemount.ps1 -Seat cdp|cdp-debug (kill+nudge+remount-wake pending)", Action: "cdp_health"),
        ...
        new(
            ...
            "rg/grep/findstr via shell (or Cursor Grep) while cdp_graphql (textHits) / cdp_buffer find / codebase_index are available — desk bypass + thick stdout tax. Bare find/cdp_search unmounted from ListTools (ADR-0233 L3).",
            ["rg", "grep", "findstr", "shell", "search", "find", "codebase_index", "cdp_graphql", "textHits"],
            ["Prefer cdp_graphql op=query textHits (or like=) over shell/Cursor Grep", "cdp_buffer find for open-buffer only; shell only if GraphQL find dead"],
            [
                new("cdp_graphql op=examples — goldens 1–8", Action: "cdp_graphql"),
                new("cdp_graphql op=query textHits(query|like, scope, path)", Action: "cdp_graphql"),
                new("external tree: textHits scope=external path=", Action: "cdp_graphql"),
            ...
            [],
            "Am I searching through cdp_graphql — or reinventing archaeology in shell?"),
        new(
            "files-via-desk",
            "abnormal",
