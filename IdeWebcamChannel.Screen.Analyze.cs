...
internal static partial class IdeWebcamChannel
{
    ...
    static object Analyze(SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
        ...
        var burstDirInput = Opt(args, "burst_dir") ?? Opt(args, "images_dir") ?? Opt(args, "dir") ?? throw new ArgumentException("burst_dir is required (folder of frames).");
        var burstDir = Path.IsPathRooted(burstDirInput) ? Path.GetFullPath(burstDirInput) : Path.GetFullPath(Path.Combine(workspaceRoot, burstDirInput));
        ...
        var allFrames = Directory.EnumerateFiles(burstDir).Where(path => ImageExtensions.Contains(Path.GetExtension(path))).OrderBy(path => path, StringComparer.OrdinalIgnoreCase).ToList();
        if (allFrames.Count == 0)
            throw new ArgumentException("No image files found in burst_dir.");
        var sampledFrames = allFrames.Where((_, index) => index % sampleEvery == 0).Take(maxFrames).ToList();
        ...
        for (var i = 1; i < sampledFrames.Count; i++)
        {
            ...
            timeline.Add(new { from = Path.GetFileName(previousFile), to = Path.GetFileName(currentFile), motion_score = Math.Round(motionScore, 2), motion_level = level, is_scene_cut = isCut });
        ...
        var topPeaks = peaks.OrderByDescending(p => p.Score).Take(5).Select(p => new { frame = p.Frame, motion_score = Math.Round(p.Score, 2), motion_level = p.Level }).ToList();
        var sceneCuts = timeline.Where(t =>
        {
            var el = JsonSerializer.SerializeToElement(t);
            return el.TryGetProperty("is_scene_cut", out var c) && c.ValueKind == JsonValueKind.True;
        }).ToList();
        var summary = $"Analyzed {sampledFrames.Count} sampled frames from {allFrames.Count} total. " + $"Motion avg={avgMotion:F2}, min={minMotion:F2}, max={maxMotion:F2}. " + $"Scene cuts detected: {sceneCuts.Count}.";
        return new
        {
            schema = Schema,
            ok = true,
