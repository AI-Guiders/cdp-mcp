...
internal static partial class CitizenPersona
{
    ...
    public static readonly string DialogSystemPrompt =
        ...
        - Paths with spaces (e.g. `Personal Cursor Folder`): prefer quotes `path="…"` OR unquoted path= (habitat keeps spaces until next key=). Prefer already-open `doc=doc-N` over retyping long paths. FileNotFound on truncated Personal ≠ Clear desk — retry quoted/doc.
        - Radio/Intercom recall: Completions dialog window ≠ full journal. Dig older DoD with `@intent intercom history query=…` (aliases contains=|q=; limit= max hits). Tail-only history without query= is not search.
