...
{
    static List<object> Enumerate(string cwd, IReadOnlyDictionary<string, JsonElement> args, int cap, out int total, out bool truncated)
    {
        ...
        try
        {
            ...
            items = di.EnumerateFileSystemInfos().OrderBy(x => x is not DirectoryInfo).ThenBy(x => x.Name, StringComparer.OrdinalIgnoreCase);
        }
        catch (Exception)
        {
            total = 0;
    ...
    static object EntryFrom(FileSystemInfo info) => info switch
    {
        DirectoryInfo d => new
        {
            kind = "dir",
    ...
    static bool IsHidden(FileSystemInfo info) => info.Name.StartsWith('.') || (info.Attributes & FileAttributes.Hidden) != 0;
    static string NormShape(string? raw) => (raw ?? "slim").Trim().ToLowerInvariant() switch
    {
        "list" or "full" => "list",
        "raw" => "raw",
        _ => "slim"
