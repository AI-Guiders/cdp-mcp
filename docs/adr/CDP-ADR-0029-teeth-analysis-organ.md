...
4. Partner: first HILD away = status + schedule escalate (~60s); still away → `SetAutonomous(true)` **and** one-shot wake `hild-escalate-away` (`charge_mode=escalate`, composer lead `reason=escalate`). Claim `AwayEscalateDone` under one `HildGate` lock (no TOCTOU storm); stable arm id replaces prior like `leaf-wake`. Autonomy latch alone is not a wake — agent must receive Composer charge if the first away turn already ended.
