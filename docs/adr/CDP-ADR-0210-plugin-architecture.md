...
1. **Modeling-провайдеры** (guiders-fsharp, Modeling.Ide.Session): `ISolutionInfoProvider {Name, Fingerprint, Entries, Relations}` — MsBuild (slnx/sln/csproj), FileGraph (md/json/toml/yaml). Каждая новая реализация = порт-проект в билде + slnx + пересборка ядра.
...
- Порты саморегистрируются явным composition-root init: `Ports.DotNet.Registration.init()`, `Ports.Files.Registration.init()`.
