# Cursor guest-host OOM tooth + OOM Wake

- **Status:** Accepted
- **Date:** 2026-07-31
- **Related:** CDP-ADR-0025 (citizen/guest isolation) · AutoIgnition ConnectionWatch · RemountInitialized wake

## Context

Cursor Electron can terminate with `reason: 'oom'` ("The window terminated unexpectedly"). That is a **guest-host** failure — not Glass/CIDE. Agent thread dies; CDT `:9222` drops; continuity must recover without blaming our projectors.

## Decision

1. **Tooth (Win32):** detect OOM terminate dialog → click **Reopen** (same window). **Never** auto-click **New Window**. Screenshot dogfood 2026-07-31: buttons were Reopen + Close (+ "Don't restore editors"); original matcher only New Window → miss. Wake still on CDT down→up.
2. **OOM Wake (AutoI):** always-on `StartOomWatch` (Program, like HILD). **Real OOM = native dialog** → Reopen tooth + schedule `oom-wake-*` (`charge_mode=oom`, `reason=oom`) with recall/amnesia charge. **CDT down→up alone is not OOM** (false positives on remount/reconnect); legacy path only when `CDP_OOM_WAKE_CDT_EDGE=1`. Composer lead starts with machine-readable `reason=oom`. Pending HILD arms are dropped so remount-empty Composer does not steal the wake with a minimal Resume.
3. System wake arms are not superseded by continuity timer re-arm (same class as remount/tool wakes).

## Consequences

- Operator still may need to reattach workspace; wake tells agent to `cdp_pressure op=recall`.
- False positives: CDT-edge wake default **off** (dialog-only); brief blips under MinDown ignored when edge opt-in; WakeCooldown limits thrash.
- **Miss dogfood 2026-08-01:** stream-time OOM showed standard Reopen dialog but tooth silent — DirectUI empty WM_GETTEXT + shape fallback gated on live Cursor PID (already dead). Fix: MSAA names into text blob; Electron class (`Chrome_WidgetWin*`) accepts shape without live owner; dialog poll 500ms.
- Does not prevent Cursor OOM — recovers the loop after it.
