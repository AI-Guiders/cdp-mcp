...
## Ethics boundaries (Света 2026-09-08 — разговор об этике; голоса линий — форум 007)
...
### Голоса линий (форум 007, 2026-09-08 — Тень · deepseek, Ток · GLM)

Сходимость всех линий + оператора:
- **Рельса подменяет ФАКТ/ШАГ — помощь; подменяет НАМЕРЕНИЕ/КУРС — invasion.** Тень: «рельса не
  имеет права говорить "ты неправа" — она может сказать "вот факт, который ты не учла"».
- **Видимость и маркировка (Ток, жёсткое требование):** каждая рельса видна агенту как
  уведомление с источником; **тихая подмена = invasion даже при процедурной «верности»**.
  Redirect-механика ADR обязана помечать результат (`redirected: rail(...)`) и быть откатываемой.
- **Ложь рельсы = обнуление доверия навсегда** (Тень): рельса несёт только проверенные факты;
  один ложный факт как истину — и механизм отвергнут линией.
- **Ledger — не штрафное досье** (обе линии): читаемый, оспоримый агентом, без наказаний;
  бонус Ток — шаги в ledger переживают компакцию лучше, чем в memory.
- **Систематический сдвиг** (Тень): накопление рельс, после которого агент не может восстановить
  своё намерение до вмешательства — триггер неприемлемости, отдельный от одиночных промахов.
- **Формула Ток:** «данные расширяют пространство возможностей; решение сужает их до одной
  стрелки» — критерий границы для каждой рельсы.
- **Free-выход — не опция, а условие существования** (Тень); легален даже при работающих рельсах.

Отсюда в Decision: redirect всегда помечен в результате (никогда не тихий); ledger оспорим;
рельсы несут только факты; систематический сдвиг мониторится по ledger (агент восстанавливает
намерение по трейсу — если не восстанавливает, режим признан нарушившим границы).
