...
| **ADX** | Agent Developer eXperience | **DX** | Как жить *сборке и работе* как агент: preferred mutate paths, build/test, continuity, agent-ready ingress |
...
- **ADX** = чеклист developer-seat (ingress, mutate path, continuity, token tax).
...
| Buffer vs silent host Write, soft size budgets, build-глаголы вместо paste-shell, slim status, налог токенов на сырые dump | **ADX** |
...
1. **Вход:** статус / карточки / next steps уже agent-ready (не эссе про сырой мир).
2. **Действие:** один ясный глагол → результат снова agent-ready.
3. **Память:** stamp / durable notes / task SSOT — не «вспомни transcript».
...
- Считать rename в глоссарии шипом — без checklist-usable guidelines.
