...
**Don't:** reach for bare `find` / `find_in_files` / `cdp_peek` / `cdp_search` / `cdp_goto` / `get_diagnostics` — unmounted from ListTools same release. Engines remain in-proc for GraphQL resolvers. Soft QRH `find-via-desk` + ECL `find-desk` point at `cdp_graphql`.
