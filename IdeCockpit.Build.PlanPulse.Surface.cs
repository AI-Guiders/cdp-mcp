...
{
    static DeskProbeBundle CollectPlanPulseProbeBundle(SessionContext session, IntentWorkspaceStore? workspaceStore, IntentWorkspaceState workspaceState)
    {
        ...
        var problems = new IdeProblemsChannel.Snap(true, "problems · pulse", 0, 0, 0, 0, 0, Array.Empty<IdeProblemsChannel.Row>());
        var chkCtx = IdeChkChannel.CtxFrom(session, workspaceState.ActiveStageId is not null, !IdeIgniteArmHost.HasContinuityArms(), gitKnown: false, gitDirty: false, testsGreen: test is { Available: true, LastRun: not null, Success: true }, testsFailed: test is { Available: true, LastRun: not null, Success: false }, problemsClean: true, dapStopped: debug.Stopped, dapActive: debug.ActiveDap, sniperOk: true);
    ...
    static (IdeAlertChannel.Snap AlertSnap, IdeAlertChannel.Inputs AlertInputs) ApplyPlanPulseGlass(SessionContext session, IntentWorkspaceStore? workspaceStore, IntentWorkspaceState workspaceState, BufferSnap buffer, ShellSnap shell, DeskProbeBundle probes)
    {
        ...
        var alertInputs = new IdeAlertChannel.Inputs(probes.Quality, DiskChanged: buffer.DiskChangedCount, DapActive: probes.Debug.ActiveDap, DapStopped: probes.Debug.Stopped, ProblemErrors: 0, ProblemWarnings: 0, ShellRunning: shell.Running, ShellFailed: shell.Failed, GitDirty: false, Sit: null, StagePhaseMismatch: null, ChkOpenRequired: probes.ChkSnap.OpenRequired, ChkPulse: probes.ChkSnap.Pulse);
    ...
    static string FinishPlanPulseDesk(SessionContext session, DocumentBufferStore docStore, ShellHabitat shellHabitat, InternetBrowserHabitat internetBrowser, IdeSettingsHabitat ideSettings, IntentWorkspaceStore? workspaceStore, IntentWorkspaceState workspaceState, IReadOnlyDictionary<string, JsonElement> args, string mfd, string? focusId, object? goResult, object? warm)
    {
        ...
        var(alertSnap, _) = ApplyPlanPulseGlass(session, workspaceStore, workspaceState, buffer, shell, probes);
        goResult = SlimGoResult(goResult, OptString(args, "go_detail"));
        var(loci, next, focus, goVerbs) = BuildDeskNavigation(session, git: null, shell, browser, settingsPulse, buffer, probes.Debug, probes.Test, probes.Work, probes.Quality, focusId, alertSnap, probes.ChkSnap, probes.ChkCtx);
        if (IdeDeskSeats.IsSeatsMode())
        {
            return BuildDeskPulseSeatsSurface(session, args, mfd, focusId, goResult, warm, next, focus, alertSnap, loci, goVerbs, resultPin: "plan");
        }

        return ComposeTilesSurface(session, mfd, tiles: null, pins: Array.Empty<string>(), goResult, warm, next, focus, alertSnap, loci, goVerbs, args, focusId);
    }

    /// <summary>
    /// Organ-only pulse: deferred soft want, no residual goVerb — board + slim go, skip nav.
    /// </summary>
    static string FinishOrganPulseDesk(SessionContext session, DocumentBufferStore docStore, ShellHabitat shellHabitat, IntentWorkspaceStore? workspaceStore, IntentWorkspaceState workspaceState, IReadOnlyDictionary<string, JsonElement> args, string mfd, string? focusId, object? goResult, object? warm, DeferredSoftWants deferred)
    {
        var buffer = CollectBuffer(docStore.Scene());
        var shell = CollectShell(shellHabitat.Scene());
        var probes = CollectPlanPulseProbeBundle(session, workspaceStore, workspaceState);
        ...
        if (IdeDeskSeats.IsSeatsMode())
        {
            return BuildDeskPulseSeatsSurface(session, args, mfd, focusId, goResult, warm, next, focus: null, alertSnap, loci: Array.Empty<Locus>(), goVerbs: Array.Empty<string>(), resultPin);
        }

        return ComposeTilesSurface(session, mfd, tiles: null, pins: Array.Empty<string>(), goResult, warm, next, focus: null, alertSnap, loci: Array.Empty<Locus>(), goVerbs: Array.Empty<string>(), args, focusId);
    }
    ...
    static string? TryGoPinFromResult(object? goResult)
    {
        if (goResult is null)
            return null;
        if (goResult is JsonElement je && je.ValueKind == JsonValueKind.Object && je.TryGetProperty("go", out var g) && g.ValueKind == JsonValueKind.String)
    ...
    static string BuildDeskPulseSeatsSurface(SessionContext session, IReadOnlyDictionary<string, JsonElement> args, string mfd, string? focusId, object? goResult, object? warm, object next, object? focus, IdeAlertChannel.Snap alertSnap, IReadOnlyList<Locus> loci, string[] goVerbs, string? resultPin)
    {
        ...
        foreach (var seatId in IdeDeskSeats.Order)
        {
            ...
            if (goResult is not null && wantPin is { Length: > 0 } && pin == wantPin)
            {
            ...
            {
                seatPanes.Add(new SeatPane(seatId, organ, false, false, true, IdeDeskView.ShortOrgan(organ), null));
            }
        }

        return ComposeSeatsSurface(session, mfd, seatPanes, wantPanes: false, seatPinList, goResult, warm, next, focus, alertSnap, DeskPulseWSprayThrash(args), loci, goVerbs, args, focusId);
    }
}