...
internal static partial class IdeTaskManager
{
        ...
        string? executor = null)
    {
        ...
        if (store.FindStageMatching(state, title, parentId, matchParent: true) is { } existing)
        {
            ...
            ApplyExecutorIfPresent(store, state, existing, executor);
            return new
            {
                op = "task_focus",
                task_id = existing,
                ...
                executor = IntentWorkspaceStore.NormalizeExecutor(executor),
                deduped = true
            };
        }
        ...
        ApplyExecutorIfPresent(store, state, r.stage_id, executor);
