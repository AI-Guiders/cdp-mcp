...
internal static partial class IdeToolchainChannel
{
    static object Ensure(IReadOnlyDictionary<string, JsonElement> args)
    {
            ...
            return new
            {
                ...
                next = new object[]
                {
                    ...
                    new
                    {
                        go = "shell_scene",
                        label = "Shell",
                        why = "manual install then toolchain_probe"
                    },
                    new
                    {
                        go = "toolchain_add",
                        label = "Register recipe",
                        why = "op=add id=… bins=…"
                    }
                },
        ...
        return new
        {
            ...
            after = RowCard(after),
            next = after.Ok ? NextAfterOk(after) : new object[]
            {
                new
                {
                    go = "internet_browser_search",
                    label = "Search",
                    why = $"q={recipe.SearchQuery}"},
                new
                {
                    go = "shell_last",
                    label = "Shell last",
                    why = "see install output"
                },
                new
                {
                    go = "toolchain_install",
                    label = "Retry",
                    why = $"id={id} via={via}"}
            },
            hint = after.Ok ? "Toolchain ready on PATH." : "Install ran — if still missing, PATH may need MCP remount / new shell."
        };
    }
    ...
    static object Add(IReadOnlyDictionary<string, JsonElement> args)
    {
        ...
        var argv = ReadStringArray(args, "argv") ?? ReadStringArray(args, "args") ?? ["winget", "install", "-e", "--id", id];
        var recipe = new Recipe(id, Opt(args, "label") ?? id, bins, search, pairsLsp, [new ViaStep(via, argv)]);
        ...
        return new
        {
            ...
            pairs_lsp = pairsLsp,
            probe = RowCard(probe),
            hint = probe.Ok ? "User recipe registered + resolves." : "Registered — op=ensure / shell then probe."
        };
    }
