...
internal static partial class IdeOnboardChannel
{
    ...
    static void CollectDocs(string root, DocsHint docs)
    {
        ...
        docs.HasDocsDir = Directory.Exists(Path.Combine(root, "docs")) || Directory.Exists(Path.Combine(root, "Docs"));
        var adr = 0;
        ...
        )
        {
            ...
            try
            {
                foreach (var f in Directory.EnumerateFiles(dir, "*.md", SearchOption.AllDirectories))
                {
                    ...
                    if (name.Contains("adr", StringComparison.OrdinalIgnoreCase) || dir.Contains("adr", StringComparison.OrdinalIgnoreCase))
                    {
                        adr++;
                        if (adr >= 50)
                            break;
    ...
    static string DetectProfileHint(string root)
    {
        ...
        if (Directory.Exists(Path.Combine(cockpit, "Channels")) && Directory.Exists(Path.Combine(cockpit, "Composition")) && (Directory.Exists(Path.Combine(root, "IdeDisplay")) || Directory.Exists(Path.Combine(cockpit, "Cds"))))
            return "cide";
        if (File.Exists(Path.Combine(root, "IdeCockpit.cs")) && File.Exists(Path.Combine(root, "IdeCockpit.Build.cs")))
            return "cdp_desk";
        return "unknown";
    }
    ...
    static int ScoreEntrypoint(string fileName)
    {
        ...
        if (fileName.StartsWith("App.", StringComparison.OrdinalIgnoreCase))
            return 90;
        if (fileName.Contains("CompositionRoot", StringComparison.OrdinalIgnoreCase))
            return 85;
        if (fileName.Contains("Startup", StringComparison.OrdinalIgnoreCase))
            return 80;
        if (fileName.Contains("Host", StringComparison.OrdinalIgnoreCase) && !fileName.Contains("Hosting", StringComparison.OrdinalIgnoreCase))
            return 70;
        if (fileName.Contains("MainWindow", StringComparison.OrdinalIgnoreCase))
            return 60;
        if (fileName.Contains("Bootstrap", StringComparison.OrdinalIgnoreCase))
            return 55;
        return 40;
    }
    ...
    static bool IsNoiseVertical(string name) => name.Equals("tests", StringComparison.OrdinalIgnoreCase) || name.EndsWith(".Tests", StringComparison.OrdinalIgnoreCase) || name.Equals("properties", StringComparison.OrdinalIgnoreCase) || name.Equals("resources", StringComparison.OrdinalIgnoreCase);
