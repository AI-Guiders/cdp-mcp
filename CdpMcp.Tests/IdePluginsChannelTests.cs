...
public sealed class IdePluginsChannelTests
{
    [Fact]
    ...
    public void Search_then_install_by_id_via_stub_handler()
    {
        ...
        finally
        {
            ...
            try { Directory.Delete(pluginsRoot, true); } catch { /* ignore */ }
        }
    }
    ...
    static byte[] MakeTinyVsixBytes()
    {
        ...
        Directory.CreateDirectory(ext);
        File.WriteAllText(Path.Combine(ext, "package.json"),
            """{"name":"plantuml","displayName":"PlantUML","version":"2.18.1","publisher":"jebbs"}""",
            Encoding.UTF8);
        // >= 64 bytes for download_empty guard
        File.WriteAllBytes(Path.Combine(ext, "plantuml.jar"),
            Encoding.ASCII.GetBytes("PK fake-jar-bytes-padded-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"));
        var vsix = Path.Combine(Path.GetTempPath(), "cdp-vsix-" + Guid.NewGuid().ToString("N") + ".vsix");
        if (File.Exists(vsix)) File.Delete(vsix);
        ZipFile.CreateFromDirectory(work, vsix);
        var bytes = File.ReadAllBytes(vsix);
        try { Directory.Delete(work, true); } catch { /* ignore */ }
        try { File.Delete(vsix); } catch { /* ignore */ }
        return bytes;
    }
