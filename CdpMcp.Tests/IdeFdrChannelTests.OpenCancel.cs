...
public sealed partial class IdeFdrChannelTests
{
    [Fact]
    public void Open_lists_start_without_close_ghost()
    {
        ...
        try
        {
            ...
            Assert.Contains(open, o =>
            {
                ...
                return doc.RootElement.GetProperty("call").GetString() == "ghostcall01" && doc.RootElement.GetProperty("tool").GetString() == "cdp_csx_run";
            });
            IdeFlightDataRecorder.RecordToolCall("cdp_csx_run", "ghostcall01", args, 45, elapsedMs: 12, outcome: "cancel", wakeExceeded: false, error: "host_abort", resultChars: 0);
            Assert.DoesNotContain(IdeFlightDataRecorder.ListOpenFlights(20), o => JsonSerializer.Serialize(o).Contains("ghostcall01", StringComparison.Ordinal));
            var session = new SessionContext
            {
                ProjectRoot = iso
            };
            var payload = IdeFdrChannel.Handle(session, new Dictionary<string, JsonElement>(StringComparer.Ordinal) { ["op"] = JsonSerializer.SerializeToElement("open") });
        ...
        finally
        {
            ...
            try
            {
                Directory.Delete(iso, recursive: true);
            }
            catch
            { /* best-effort */
            }
    ...
    public async Task Cancel_records_start_and_closed_cancel()
    {
        ...
        try
        {
            ...
            var run = IdeToolCallWatch.RunAsync("cdp_health", args, async ct =>
            {
            ...
            Assert.Contains(tail, e => e.Tool == "cdp_health" && e.Outcome == "cancel" && e.Kind == IdeFlightDataRecorder.KindToolCall);
    ...
    public async Task Mid_flight_ticks_on_tape_and_excluded_from_stats()
    {
        ...
        try
        {
            ...
            var text = await IdeToolCallWatch.RunAsync("cdp_health", args, async ct =>
            {
            ...
            Assert.Contains(tail, e => e.Kind == IdeFlightDataRecorder.KindToolTick && e.Outcome == IdeFlightDataRecorder.OutcomeRunning && e.ElapsedMs >= 900);
            ...
            var stats = IdeFdrChannel.Handle(session, new Dictionary<string, JsonElement>(StringComparer.Ordinal) { ["op"] = JsonSerializer.SerializeToElement("stats") });
            ...
            var trace = IdeFdrChannel.Handle(session, new Dictionary<string, JsonElement>(StringComparer.Ordinal) { ["op"] = JsonSerializer.SerializeToElement("trace"), ["call"] = JsonSerializer.SerializeToElement(callId) });
    ...
    public void Open_includes_last_tick_when_present()
    {
        ...
        try
        {
            ...
            IdeFlightDataRecorder.RecordToolTick("cdp_csx_run", "tickghost01", args, 45, elapsedMs: 12_000, wakeExceeded: false);
            var open = IdeFlightDataRecorder.ListOpenFlights(20);
            Assert.Contains(open, o =>
            {
                var json = JsonSerializer.Serialize(o);
                using var doc = JsonDocument.Parse(json);
                return doc.RootElement.GetProperty("call").GetString() == "tickghost01" && doc.RootElement.GetProperty("last_tick_ms").GetInt32() == 12_000 && doc.RootElement.GetProperty("ticks").GetInt32() == 1;
            });
        }
        finally
        {
    ...
    public async Task Watchdog_ghost_cancel_closes_hung_call()
    {
        ...
        try
        {
            ...
            await Assert.ThrowsAnyAsync<OperationCanceledException>(async () => await IdeToolCallWatch.RunAsync("cdp_health", args, async ct =>
            {
                await Task.Delay(Timeout.Infinite, ct);
                return "{}";
            }, CancellationToken.None));
            ...
            Assert.Contains(tail, e => e.Tool == "cdp_health" && e.Kind == IdeFlightDataRecorder.KindToolStart);
            Assert.Contains(tail, e => e.Tool == "cdp_health" && e.Kind == IdeFlightDataRecorder.KindToolCall && e.Outcome == "cancel" && (e.Error?.Contains("ghost_cancel", StringComparison.Ordinal) ?? false));
    ...
    public void CancelOrphan_closes_tape_ghost_skips_live()
    {
        ...
        try
        {
            ...
            var result = IdeFlightDataRecorder.CancelOrphanOpenFlights(liveCallIds: ["live01"], minAgeSeconds: 0, lookback: 40);
            ...
            var close = IdeFlightDataRecorder.ReadTail(20).First(e => e.CallId == "orphan01" && e.Kind == IdeFlightDataRecorder.KindToolCall);
