...
{
    static object Pulse(DocumentBufferStore store, SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
    ...
    static object Plan(DocumentBufferStore store, SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
        ...
        var pulse = TryRecommendPulse(recommend) ?? (top is null ? "refactor_plan · no hotspots in scope" : $"refactor_plan · top {Rel(session.ProjectRoot, top.Path)} · {top.Metric}={top.Value}");
        return new
        {
            ok = true,
            schema = Schema,
    ...
    static object Recommend(DocumentBufferStore store, SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
        var debt = BuildDebt(store, session, args, max: 8);
        var budget = BuildBudget(store, session, args, debt);
        var partials = BuildPartials(session, args);
    ...
    static object[] PreferRecommendNext(object recommend, object[] fallback)
    {
        ...
        {
            using var doc = JsonDocument.Parse(JsonSerializer.Serialize(recommend));
            if (doc.RootElement.TryGetProperty("next", out var narr) && narr.ValueKind == JsonValueKind.Array && narr.GetArrayLength() > 0)
            {
                return narr.EnumerateArray().Select(n => (object)new { go = n.TryGetProperty("go", out var g) ? g.GetString() : null, label = n.TryGetProperty("label", out var lab) ? lab.GetString() : null, why = n.TryGetProperty("why", out var w) ? w.GetString() : null }).ToArray();
            }
        }
        catch
        { /* fall through */
    ...
    static object Debt(DocumentBufferStore store, SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
    ...
    static object Budget(DocumentBufferStore store, SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
        var debt = BuildDebt(store, session, args, max: 3);
        var budget = BuildBudget(store, session, args, debt);
        return new
    ...
    static object Partials(SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
        var partials = BuildPartials(session, args);
        return new
        {
