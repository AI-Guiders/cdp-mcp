...
internal static partial class IdeTeethChannel
{
    ...
    {
        var cdtLive = OptBool(args, "cdt") ?? ShouldRefreshCdtSample();
        ...
        return new
        {
            ...
            hint = "Guest-host teeth: CDT/Stop, remount·oom delivery, OOM tooth, partner away/here. " + "First away = status; still away after ~60s → autonomy. Health pulse auto-samples CDT when unknown/stale (~15s); cdt=true forces live sample."
        };
    }
    ...
    static object Explain(IReadOnlyDictionary<string, JsonElement> args)
    {
        ...
        var now = BuildNow(cdtLive: OptBool(args, "cdt") ?? ShouldRefreshCdtSample());
