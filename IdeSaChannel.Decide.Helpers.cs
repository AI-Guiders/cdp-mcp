...
{
    static object[] TakeFindings(GatesSnap gates, int max) => gates.Findings.Take(max).Select(f => (object)new { id = f.Id, severity = f.Severity, path = f.Path, symbol = f.Symbol, message = f.Message, go = f.Go }).ToArray();
    static object[] BuildNext(Locus locus, string scope, string verdict)
    {
        var list = new List<object>
        {
            ...
            new
            {
                go = "analysis_scene",
                label = "Clones / map",
                why = "feature=clones|semantic_map"
            },
            new
            {
                go = "review",
        ...
        if (locus.Line is> 0)
        {
            list.Insert(0, new { go = "find_usages", label = "Blast radius", why = $"file_path= line={locus.Line} column={locus.Column ?? 1}" });
        }
        else
        {
            list.Insert(0, new { go = "goto", label = "Land locus", why = "need line/col for find_usages" });
        }

        if (verdict is "split" or "touch")
        {
            list.Insert(0, new { go = "refactor_plan", label = "Recommended next cut", why = "op=plan|recommend — one package before extract" });
    ...
    static Locus ResolveLocus(DocumentBufferStore store, SessionContext session, IReadOnlyDictionary<string, JsonElement> args)
    {
    ...
    static int EndOfField(string s, int start)
    {
        ...
        if (semi < 0)
            return br < 0 ? s.Length : br;
        if (br < 0)
            return semi;
        return Math.Min(semi, br);
    }

    static IdeReviewChannel.FileCard? FindDirtyForLocus(IReadOnlyList<IdeReviewChannel.FileCard> dirty, string? path, string? projectRoot)
    {
        ...
        {
            var cand = projectRoot is { Length: > 0 } ? Path.GetFullPath(Path.Combine(projectRoot, d.Path.Replace('/', Path.DirectorySeparatorChar))) : Path.GetFullPath(d.Path);
