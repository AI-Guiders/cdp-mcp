#nullable enable
using static CdpMcp.IdeIgniteArmHost;

namespace CdpMcp;

/// <summary>Timer loop + arm status hygiene (≤ADX soft-warn peel).</summary>
internal sealed partial class CdpIgniteArmHost
{
    async Task TimerLoopAsync(CancellationToken ct)
    {
        using var timer = new PeriodicTimer(TimeSpan.FromSeconds(1));
        while (await timer.WaitForNextTickAsync(ct).ConfigureAwait(false))
        {
            // Autonomous + TM leaf Fly: pull long last_once parks even when partner is here.
            if (IsAutonomousArmed())
                TryPullForwardLongWorkTimersOnLeafFlyPerTenant();

            List<IgniteArm> due;
            lock (Gate)
            {
                var now = _time.GetUtcNow();
                due = Arms.Where(a =>
                        a.Status == "armed"
                        && a.Event == "timer"
                        && a.DueUtc is { } d
                        && d <= now)
                    .Select(Clone)
                    .ToList();
            }

            foreach (var arm in due)
                QueueFire(arm, ok: true, pulse: "timer", detail: arm.InRaw);
        }
    }

    void SetStatus(string id, string status, string? error, DateTimeOffset? fired = null)
    {
        lock (Gate)
        {
            var a = Arms.FirstOrDefault(x => x.Id.Equals(id, StringComparison.OrdinalIgnoreCase));
            if (a is null) return;
            a.Status = status;
            a.LastError = error;
            // FiredUtc = delivery attempt completed (ApplyFireOutcome), not "entered wait-idle".
            if (fired is { } f) a.FiredUtc = f;
            PersistUnlocked();
        }
    }


    /// <summary>
    /// last_once → awaiting_partner only when autonomous is off.
    /// Under autonomous overnight, awaiting after successful fire is ACC invent-ban.
    /// </summary>
    internal bool ShouldLatchAwaitingPartnerAfterSuccessfulFire(bool lastOnce, bool autonomousArmed) =>
        lastOnce && !autonomousArmed;

    internal bool ShouldRequeueBusy(string eventName, string? error) =>
        string.Equals(eventName, "timer", StringComparison.OrdinalIgnoreCase)
        && error is not null
        && (string.Equals(error, "busy_timeout", StringComparison.Ordinal)
            // After OOM New Window Composer mounts late — once-arm must not die silent.
            || string.Equals(error, "no_agent_composer", StringComparison.Ordinal)
            || string.Equals(error, "wrong_surface", StringComparison.Ordinal)
            // CDT Send click miss — once-arm must not die silent under Composer Stop/Queue thrash.
            || string.Equals(error, "click_failed", StringComparison.Ordinal));


    internal bool ShouldKeepVisibleErrorOnFireFail(bool once, bool lastOnce) =>
        lastOnce || !once;

    internal TimeSpan BusyBackoff(int waitSeconds) =>
        TimeSpan.FromSeconds(Math.Clamp(waitSeconds / 3, 15, 60));

    void RequeueAfterBusy(string id, string error, TimeSpan backoff)
    {
        lock (Gate)
        {
            var a = Arms.FirstOrDefault(x => x.Id.Equals(id, StringComparison.OrdinalIgnoreCase));
            if (a is null) return;
            a.Status = "armed";
            a.LastError = error;
            a.DueUtc = _time.GetUtcNow() + backoff;
            PersistUnlocked();
            IdeTeethTape.Record(
                "wake_requeue",
                armId: a.Id,
                reason: a.Reason,
                detail: error);
        }
    }

    void Remove(string id)
    {
        lock (Gate)
        {
            Arms.RemoveAll(a => a.Id.Equals(id, StringComparison.OrdinalIgnoreCase));
            PersistUnlocked();
        }

        CancelInFlightFire(id);
    }
}
